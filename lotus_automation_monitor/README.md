...
La 0.2.13 transforme le projet en **dépôt d’Apps Home Assistant directement distribuable**. Le dépôt contient désormais `repository.yaml` à sa racine et l’App est placée dans son dossier technique `lotus_automation_monitor`.

Après publication du dépôt GitHub, l’utilisateur peut ajouter directement l’adresse du dépôt dans **Paramètres → Apps → Installer une app → Dépôts**, puis installer **Lotus Colour Automation** depuis Home Assistant.

Aucun changement fonctionnel n’est apporté à la surcouche par rapport à la 0.2.12.
